 ...
 */
class SlidesListFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        slidesAdapter.items = listOf(
            ConfirmationItemDisplay("Лекция 1", "slide1.pdf", "C++ кіріспе"){
                navigateToPdfFragment(it.path)
            },
            ConfirmationItemDisplay("Лекция 2", "slide2.pdf", "C++ екінші сабақ"){
                navigateToPdfFragment(it.path)
            },
            ConfirmationItemDisplay("Лекция 3", "slide3.pdf", "C++ үшінші сабақ"){
                navigateToPdfFragment(it.path)
            },
            ConfirmationItemDisplay("Лекция 4", "slide4.pdf", "C++ төртінші сабақ"){
                navigateToPdfFragment(it.path)
            }
        )
    }
    ...
    private fun navigateToPdfFragment(path: String?){
        findNavController().navigate(R.id.pdfReader, bundleOf(
            "path" to  path
        ))
    }
