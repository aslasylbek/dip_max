...
class LectureFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        slidesAdapter.items = listOf(
            ConfirmationItemDisplay("Дәріс 1", "Dar1.pdf", "Графика және мультимедия"){
                navigateToPdfFragment(it.path)
            },
            ConfirmationItemDisplay("Дәріс 2", "Dar2.pdf", "Графикалық файлдар форматтары."){
                navigateToPdfFragment(it.path)
            },
            ConfirmationItemDisplay("Дәріс 3", "Dar3.pdf", "Графикалық ақпараттарды кескіндеу компоненттері"){
                navigateToPdfFragment(it.path)
            }
        )
    }
